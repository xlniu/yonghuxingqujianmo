...
embedding_matrix_text = pd.read_pickle('./data/text_nmf20.pkl')
embedding_matrix_text = embedding_matrix_text.astype(np.float32)
EMBEDDING_DIM_TEXT = 20
...
embedding_layer_photo = Embedding(nb_photos,
                                  ...
                                  trainable=False)
embedding_layer_text = Embedding(nb_photos,
                                  EMBEDDING_DIM_TEXT,
...
embedded_text_mean = GlobalAveragePooling1D()(embedded_text_agg)
...
model = Model(inputs=[input_user, input_photo, input_user_mean,input_feature,input_photo_mean], outputs=preds)
...
train_photo_mean = pad_sequences(train['photo_id_doc'].values, maxlen=MAX_SENTENCE_LENGTH)
...
submission.to_csv('./data/submission7.txt',sep='\t',index=False,header=False)